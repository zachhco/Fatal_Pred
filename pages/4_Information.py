...
left_col, middle, right_col = st.columns([0.3, 1.35, 0.3])

with middle:
    st.title("Information")
    st.write("Below is the code used to creat the Fatality Predictor - Classification Model. The app is a Random Forest Classification \
             model that predicts the risk of a crash being fatal or not fatal. The model consits of the following code")
with middle:
    code = '''
    import streamlit as st
    import pandas as pd
    ...
    st.set_page_config(layout="wide")
    st.sidebar.markdown("# Death Machine - Classification")
    st.sidebar.markdown("Interactive Machine Learning App 🤖")
    st.sidebar.markdown("Random forest classification approach to modeling crash risk. 🚗")
    st.markdown('# Death Machine - Classification')
